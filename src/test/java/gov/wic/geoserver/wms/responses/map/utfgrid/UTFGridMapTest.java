...
public class UTFGridMapTest {

	private static final StyleFactory sFac = CommonFactoryFinder
			.getStyleFactory(null);

	private static final Logger LOGGER = org.geotools.util.logging.Logging
			.getLogger(UTFGridMapTest.class.getPackage().getName());
	...
	public void testMapProduceReproject() throws Exception {
		...
		LOGGER.info("about to create map ctx for ProjectedPolygon with bounds "
				+ env);
	...
	public void testMapProduceLines() throws Exception {
		...
		LOGGER.info("about to create map ctx for RoadSegments with bounds "
				+ env);
	...
	public void testMapRuleWithFilters() throws Exception {
		/*
		 * Filter
		 * f=filterFactory.equals(filterFactory.property("NAME"),filterFactory
		 * .literal("Route 5")); Query q=new Query("RoadSegments",f);
		 */
		final FeatureSource<SimpleFeatureType, SimpleFeature> fs = testDS
				.getFeatureSource("RoadSegments");
		final ReferencedEnvelope env = new ReferencedEnvelope(fs.getBounds(),
	...
	public void testMapProduceMultiPoints() throws Exception {
		...
		LOGGER.info("about to create map ctx for BuildingCentersMultiPoint with bounds "
				+ env);
	...
	public void testMapProduceNoCoords() throws Exception {
		...
		final ReferencedEnvelope env = new ReferencedEnvelope(2.0, 6.0, 2.0,
				6.0, WGS84);

		LOGGER.info("about to create map ctx for NamedPlaces with bounds "
				+ env);
